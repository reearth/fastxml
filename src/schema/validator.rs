...
pub fn validate_with_schema_location(doc: &XmlDocument) -> Result<Vec<StructuredError>> {
    validate_with_schema_location_and_fetcher(doc, &super::fetcher::UreqFetcher::new())
}
...
/// ```
pub fn validate_with_schema_location_and_fetcher<F: super::fetcher::SchemaFetcher>(
    doc: &XmlDocument,
    fetcher: &F,
) -> Result<Vec<StructuredError>> {
    ...
    let store = super::memory::InMemoryStore::new();
    ...
    for (_namespace, location) in &locations {
        ...
        match fetcher.fetch(location) {
            Ok(fetch_result) => {
                ...
                match super::xsd::parse_xsd_with_imports(
            ...
            Err(_) => {
                return Err(crate::error::Error::Schema(
                    super::error::SchemaError::SchemaNotFound {
                        uri: location.clone(),
                    },
                ));
            }
...
/// ```
#[cfg(feature = "ureq")]
pub fn get_schema_from_schema_location(xml_content: &[u8]) -> Result<super::types::CompiledSchema> {
    get_schema_from_schema_location_with_fetcher(xml_content, &super::fetcher::UreqFetcher::new())
}
...
pub fn get_schema_from_schema_location_with_fetcher<F: super::fetcher::SchemaFetcher>(
    xml_content: &[u8],
    fetcher: &F,
) -> Result<super::types::CompiledSchema> {
    use super::store::SchemaStore;
    use crate::parser::parse_schema_locations;
    ...
    if locations.is_empty() {
        return Ok(super::xsd::create_builtin_schema());
    }
...
pub fn streaming_validate_with_schema_location<R: std::io::BufRead>(
    ...
    streaming_validate_with_schema_location_and_fetcher(reader, super::fetcher::UreqFetcher::new())
}
...
pub fn streaming_validate_with_schema_location_and_fetcher<
    R: std::io::BufRead,
    F: super::fetcher::SchemaFetcher + 'static,
>(
    reader: R,
    fetcher: F,
) -> Result<Vec<StructuredError>> {
...
pub struct LazySchemaValidator<F: super::fetcher::SchemaFetcher> {
    fetcher: F,
...
impl<F: super::fetcher::SchemaFetcher> LazySchemaValidator<F> {
    ...
    fn initialize_from_attributes(&mut self, attributes: &[(CompactString, CompactString)]) {
        ...
        let schema = if let Some(loc_value) = schema_location {
            ...
            for chunk in parts.chunks(2) {
                if chunk.len() == 2 {
                    ...
                    match self.fetcher.fetch(location) {
                        Ok(result) => {
                            let store = super::memory::InMemoryStore::new();
                            if let Ok(()) = super::store::SchemaStore::put(
                                &store,
                                &result.final_url,
                                &result.content,
                            ) {
            ...
            compiled.unwrap_or_else(super::xsd::create_builtin_schema)
        } else {
            super::xsd::create_builtin_schema()
        };

        self.validator = Some(StreamingSchemaValidator::new(std::sync::Arc::new(schema)));
    }
}

impl<F: super::fetcher::SchemaFetcher> crate::event::XmlEventHandler for LazySchemaValidator<F> {
    fn handle(&mut self, event: &crate::event::XmlEvent) -> Result<()> {
        // Initialize on first StartElement
        if let crate::event::XmlEvent::StartElement { attributes, .. } = event {
            if !self.initialized {
...
struct LazySchemaValidatorWithSharedErrors<F: super::fetcher::SchemaFetcher> {
    fetcher: F,
    validator: Option<StreamingSchemaValidator>,
    initialized: bool,
    shared_errors: std::sync::Arc<std::sync::Mutex<Vec<StructuredError>>>,
}

impl<F: super::fetcher::SchemaFetcher> LazySchemaValidatorWithSharedErrors<F> {
    fn new(
        fetcher: F,
        shared_errors: std::sync::Arc<std::sync::Mutex<Vec<StructuredError>>>,
    ) -> Self {
...
impl<F: super::fetcher::SchemaFetcher> crate::event::XmlEventHandler
    for LazySchemaValidatorWithSharedErrors<F>
{
    fn handle(&mut self, event: &crate::event::XmlEvent) -> Result<()> {
        // Initialize on first StartElement
        if let crate::event::XmlEvent::StartElement { attributes, .. } = event {
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    // =============================================
    // LazySchemaValidator Tests
    // =============================================

    #[test]
    fn test_lazy_schema_validator_new() {
        let fetcher = super::super::fetcher::NoopFetcher;
    ...
    #[test]
    fn test_lazy_schema_validator_handle_without_schema_location() {
        use crate::event::XmlEventHandler;

        let fetcher = super::super::fetcher::NoopFetcher;
        let mut validator = LazySchemaValidator::new(fetcher);

        // Handle a start element without xsi:schemaLocation
        validator
            .handle(&XmlEvent::StartElement {
                name: "root".into(),
                prefix: None,
        ...
        // Without schemaLocation, validator should be initialized but with empty schema
        assert!(validator.initialized);
    ...
    fn test_lazy_schema_validator_handle_with_schema_location() {
        ...
        // Should be initialized (with error recorded since NoopFetcher fails)
        assert!(validator.initialized);
    ...
    // =============================================
    // validate_with_schema_location Tests
    // =============================================
...
<root>
    ...
    #[test]
    fn test_validate_with_schema_location_with_schema_location() {
        let xml = r#"<?xml version="1.0"?>
<root xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <element>content</element>
        ...
        let result = validate_with_schema_location_and_fetcher(&doc, &fetcher);
        // NoopFetcher fails to fetch -> Err
    ...
    // =============================================
    // streaming_validate_with_schema_location Tests
    // =============================================
...
<root>
    ...
    #[test]
    fn test_streaming_validate_with_schema_location() {
        let xml = r#"<?xml version="1.0"?>
<root xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
      xsi:schemaLocation="http://example.com http://example.com/schema.xsd">
    <element>content</element>
        ...
        let result = streaming_validate_with_schema_location_and_fetcher(reader, fetcher);
        // Streaming completes but records fetch error
        assert!(result.is_ok());
        let errors = result.unwrap();
        // Should have error about schema fetch failure
        assert!(!errors.is_empty());
        assert!(
            errors
                .iter()
                .any(|e| e.message.contains("Failed to fetch schema"))
        );
    }

    // =============================================
    // get_schema_from_schema_location Tests
    // =============================================
...
<root>
    <element>content</element>
        ...
        let fetcher = super::super::fetcher::NoopFetcher;
        let result = get_schema_from_schema_location_with_fetcher(xml.as_bytes(), &fetcher);
        // No schemaLocation, returns builtin schema -> Ok
        assert!(result.is_ok());
    }
