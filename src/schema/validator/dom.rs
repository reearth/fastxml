...
impl DomSchemaValidator {
    ...
    pub fn validate(&self, doc: &XmlDocument) -> Result<Vec<StructuredError>> {
        ...
        if let Ok(root) = doc.get_root_element() {
            self.validate_node_recursive(&root, &mut errors);
        }
    ...
    fn validate_node_recursive(&self, node: &XmlNode, errors: &mut Vec<StructuredError>) {
        // Check max errors
        ...
        match node.get_type() {
            NodeType::Element => {
                self.validate_element(node, errors);

                // Validate children recursively
                for child in node.get_child_elements() {
                    self.validate_node_recursive(&child, errors);
                }
            }
            NodeType::Document => {
    ...
    fn validate_element(&self, node: &XmlNode, errors: &mut Vec<StructuredError>) {
        ...
        if let Some(elem) = elem_def {
            ...
            if let Some(flattened) = self.get_flattened_children_for_element(elem) {
                ...
                self.validate_min_occurs_batch(node, &child_counts, &flattened, errors);

                // Validate max_occurs for all children
                self.validate_max_occurs_batch(node, &child_counts, &flattened, errors);
            }
