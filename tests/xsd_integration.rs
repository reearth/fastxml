...
/// Test that all test files exist in benches/cache
#[test]
fn test_plateau_file_structure() {
    let cache_dir = std::path::Path::new("benches/cache");
    ...
    if cache_dir.exists() {
        ...
        // Try to parse first file
        if let Some(first_file) = gml_files.first() {
            let content = std::fs::read(first_file.path()).unwrap();
            let doc = fastxml::parse(&content);
        ...
        println!("Cache directory not found, skipping file structure test");
    }
}
